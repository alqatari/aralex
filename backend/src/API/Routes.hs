...
import API.EtymologyGraph qualified as EG
import API.Handlers qualified as H
...
type API =
  "api"
    ...
    :> ( "dictionaries" :> Get '[JSON] [Text]
           ...
           :<|> "irab" :> Capture "surah" Int :> Capture "verse" Int :> Get '[JSON] [IrabDetail]
       )
...
server dbConns@(DatabaseConnections dictConn' quranConn') =
  ...
  where
    ...
    getIrab :: Int -> Int -> Handler [IrabDetail]
    getIrab surahNum verseNum = liftIO $ H.lookupIrab quranConn' surahNum verseNum
